/**
 * LXD container image presets (v1.4 Fase 4.1). Instead of forcing the
 * operator to type "ubuntu:24.04", the container form offers a dropdown
 * of friendly, descriptive distribution labels backed by valid LXD image
 * references — every ref here passes parseImageRef() on the backend
 * (known remotes: ubuntu, ubuntu-daily, images, almalinux, rockylinux).
 * "Custom / Other…" reveals the manual text input for anything else.
 */

export const LXD_IMAGE_PRESETS = [
...
export function labelForImage(ref) {
  const hit = LXD_IMAGE_PRESETS.find((p) => p.ref === ref);
