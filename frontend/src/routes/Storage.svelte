<script>
  ...
  async function load() {
    ...
    try {
      ...
      if (!selectedISOPool || selectedISOPool === '__all__' || !isoPools.find((p) => p.name === selectedISOPool)) {
        selectedISOPool = '__all__';
      }
      isos = (await api.listISOs(selectedISOPool === '__all__' ? undefined : selectedISOPool)) || [];
    } catch (e) {
      error = e.message;
    } finally {
      loading = false;
